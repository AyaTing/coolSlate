...
async def get_current_user(request: Request, credentials:Optional[HTTPAuthorizationCredentials] = Depends(security), db: asyncpg.Connection = Depends(get_connection)):
    if not credentials:
        return None
    try:
        ...
        user_data = await db.fetchrow(select_query, user_id,)
        if not user_data:
            return None
...
async def verify_google_token(id_token: str):
    try:
        ...
        raise HTTPException(
            status_code=401, 
            detail=f"無效的 Google Token: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, 
